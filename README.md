...
This repository primarily adds Chinese translations to the configuration file and modifies the default settings to support Pixel Shader 3.0 compilation.

这是一个针对tModLoader的xnb编译器。它是泰拉瑞亚社区中广泛使用的“fxcompiler”可执行文件的更新和扩展版本，用于编译着色器。不过，此版本不仅支持编译着色器（.fx），还支持编译模型（.fbx）、纹理（.png / .bmp / .jpg / .tga / .dds）和字体（未测试）。除了其他可用性改进，如配置文件外，该编译器还引用了常规的XNA框架dll，但包含了XNA管道dll。

本仓库主要对配置文件添加中文翻译，并且修改默认配置以支持Pixel Shader3.0的编译。